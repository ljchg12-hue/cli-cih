...
VALID_AI_NAMES = {"codex", "gemini", "ollama", "copilot"}  # No claude - handled by session
...
async def cih_quick(prompt: str, ai: str = "ollama", timeout: int = 60) -> dict[str, Any]:
    """Quick external AI response (codex/gemini/ollama/copilot)"""
    start = time.time()
    try:
        if not prompt or not prompt.strip():
            raise MCPValidationError("Empty prompt")
        if ai not in VALID_AI_NAMES:
            raise MCPValidationError(f"Invalid AI: {ai}. Use: codex, gemini, ollama, copilot")
        if ai == "codex":
...
async def cih_discuss(prompt: str, ais: list[str] | None = None, max_rounds: int = 2, include_synthesis: bool = True, timeout: int = 90) -> dict[str, Any]:
    """Multi external AI discussion (codex/gemini/ollama/copilot)"""
    start = time.time()
    try:
        task = _task_analyzer.analyze(prompt)
        if ais is None:
...
async def cih_compare(prompt: str, ais: list[str] | None = None, timeout: int = 90) -> dict[str, Any]:
    """Compare external AI responses (codex/gemini/ollama/copilot)"""
    if ais is None:
        ais = ["codex", "gemini", "ollama"]
...
async def cih_status() -> dict[str, Any]:
    """Check external AI status (codex/gemini/ollama/copilot)"""
    start = time.time()
    status = {}
    ...
    return make_response(True, data={"ais": status, "summary": {"total_ais": 4, "available": available_count}, "note": "Claude handled by current session"}, duration_ms=int((time.time() - start) * 1000))
...
async def cih_smart(prompt: str, task_type: Literal["code", "debug", "design", "research", "explain", "general"] | None = None, timeout: int = 90) -> dict[str, Any]:
    """Smart routing to external AI (codex/gemini/ollama/copilot)"""
    start = time.time()
    try:
...
async def cih_models() -> dict[str, Any]:
    """List external AI models (codex/gemini/ollama/copilot)"""
    start = time.time()
    try:
        models = {}
