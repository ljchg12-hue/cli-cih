...
class Config(BaseModel):
    ...
    availability_cache_ttl: float = Field(
        default=30.0, ge=5.0, le=300.0, description="TTL for adapter availability cache in seconds"
    )
    parallel_check_timeout: float = Field(
        default=5.0, ge=1.0, le=30.0, description="Timeout for parallel adapter availability checks"
    )
