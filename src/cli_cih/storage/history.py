...
class HistoryStorage:
    ...
    async def save_session(self, session: Session) -> str:
        ...
        with self._get_connection() as conn:
            ...
            conn.execute('''
                ...
                (id, user_query, task_type, participating_ais, total_rounds, status, created_at, updated_at)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?)
            ''', (
                session.id,
                session.user_query,
            ...
            for msg in session.messages:
                conn.execute('''
                    ...
                    (id, session_id, sender_type, sender_id, content, round_num, metadata, created_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                ''', (
                    msg.id,
                    msg.session_id,
    ...
    async def get_stats(self) -> dict:
        ...
        with self._get_connection() as conn:
            ...
            ai_usage = {}
            rows = conn.execute(
                'SELECT participating_ais FROM sessions'
            ).fetchall()
